...
2. (Опц.) если прокси данных живёт не на `https://mpgu-rasp.workers.dev`,
...
4. Зарегистрируйте вебхук (один раз):
   ```sh
   curl "https://api.telegram.org/bot<BOT_TOKEN>/setWebhook?url=https://mpgu-rasp-bot.<subdomain>.workers.dev&secret_token=<WEBHOOK_SECRET>"
   ```
